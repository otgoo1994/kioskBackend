...
exports.getProperties = async (req, res) => {
  ...
  db.query(sql.selectProperties(intent, usage, city, detail, 1), async (err, normal) => {
    console.log('query=====', sql.selectProperties(intent, usage, city, detail, 1));
    if(err) {
      res.status(500).json({
...
exports.getFavProps = async (req, res) => {
  const userId = req.payload.id;
  db.query(sql.getFavProps(userId), async (err, props) => {
    if (err) {
      throw err;
    }
...
exports.removeFavProps = async (req, res) => {
  const userId = req.payload.id;
  const { id } = req.body;

  db.query(sql.removeFavProps(id, userId), async err => {
    if (err) {
      throw err;
    }
...
exports.addFavPrpos = async (req, res) => {
  ...
  db.query(sql.checkFavExisted({ userId: key.userId }), async (err, check) => {
    if (err) {
      throw err;
    }
    if (check.length > 4) {
...
async function photoUpload(file, propertyId) {
  ...
  file.mv("./public/gazar/images/property/" + iname, function (err, result) {
    if(err) {
      throw err;
    }
...
exports.sendReport = async (req, res) => {
  ...
  db.query(sql.sendReport(propertyId, title, description), async (err) => {
    if (err) {
      throw err;
    }
